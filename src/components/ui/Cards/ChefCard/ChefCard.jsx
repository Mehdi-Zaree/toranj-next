    ...
    return (
        <>
            <div className={'w-full h-full bg-white rounded-2xl flex flex-col justify-center items-center gap-4 py-8 px-8 shadow hover:shadow-2xl duration-200 '}>
                ...
                <span className={'text-2xl font-semibold'}>{Number(product.price).toLocaleString('fa')} تومان </span>
                <button
                    onClick={()=>{
                        alert(`${product.productName} به سبد خرید اضافه شد   `)
                        addToCart(product)
                    }}
                    className={'w-full h-20 bg-[#ededed] rounded-2xl text-2xl hover:bg-[#193d3d] hover:text-white font-semibold text-gray-400 duration-200'}>افزودن به سبد خرید</button>
            </div>
        </>
    );
}
